...
* Functional Verification: A combination of HDL simulation and formal
  verification to ensure that the design behaves as expected against the
  requirements. It is essential to perform functional verification on both the
  input RTL and the gate-level {term}`netlist` for the circuit.
...
* Layout/Physical Synthesis: Also called Physical Implementation. In this step,
  ...
  Routing", or {term}`PnR`.
* Signoff: The final stage in the rigorous journey of an ASIC's design; it
...
1. Run the smoke test to ensure everything is fine. As part of the test, the
   open-source {term}`sky130` {term}`process design kit <PDK>`.
...
In general, for any design, at a minimum you need to specify the following
variables:
* {var}`::DESIGN_NAME`
...
1. Configure the pin placement as follows.
